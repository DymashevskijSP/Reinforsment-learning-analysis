...
pop = load_object('Models/BipedalModels/BipedalmoreNeurons.pkl')
bestNeuralNets = []
...
for nn in pop.population:
    ...
    for attempt in range(3):
        ...
        #env.add_pits(150)
        #env.add_stumps(80)
        # Create enviroment you need here
        for step in range(MAX_STEPS):
            ...
            if done:
                break
    rend = False
    nn.fitness = totalReward/3
    minFit = min(minFit, nn.fitness)
    print("average fitness:",nn.fitness)
...
for i in range(3):
    ...
    for step in range(4*MAX_STEPS):

        env.render()
        action = maxNeuralNet.getOutput(observation)
        observation, reward, done, info = env.step(action)
        if done:
            break
