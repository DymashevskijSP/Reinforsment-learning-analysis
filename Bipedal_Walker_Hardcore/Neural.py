...
MUTATION_RATE = 0.03
from Bipedal_Walker_Hardcore.Customizers.Advanced_customizer import CustomizableBipedalWalker
env = gym.make("BipedalWalker-v2")
#env = CustomizableBipedalWalker()
#env.create_default_terrain()
#env.add_pits(30)
observation = env.reset()
...
pop = Population(POPULATION_COUNT, MUTATION_RATE, [in_dimen, 50, 30, 50, out_dimen])
#pop = load_object('Models/Custom_models/PitsMoreNeurons/model.pkl')
bestNeuralNets = []
#env.set_env_params(pit_width=(2,4), stump_height=(1,3))
target_update = 3000
for gen in range(MAX_GENERATIONS):
    ...
    for nn in pop.population:
        ...
        for attempt in range(1):
            observation = env.reset()
            #env.create_default_terrain()
            #env.add_pits(min(140, 40 +3*gen))
            #env.add_stumps(min(155, 50 + gen))
            for step in range(MAX_STEPS):
                if(rend):
                    env.render()
...
out = 'Models/BipedalModels/BipedalManyNeurons.pkl'
