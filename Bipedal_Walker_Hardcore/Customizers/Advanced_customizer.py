...
class CustomizableBipedalWalker(BipedalWalker):
    ...
    def __init__(self):
        self.GRASS, self.STUMP, self.STAIRS, self.PIT, self._STATES_ = range(5)
        self.terr = None
        self.default_params = {
            'stump_height_low': 1,
            'stump_height_high': 3,
    ...
    def _set_terrain_(self, terr=None):
        self.terr = [x for x in terr]
    def create_default_terrain(self):
        self._set_terrain_([self.GRASS for _ in range(0, TERRAIN_LENGTH)])
    def add_stumps(self, num):
        #print(len(self.terr))
        for i in range(num):
            self.terr[random.randint(0,TERRAIN_LENGTH-1)] = self.STUMP
    def add_pits(self, num):
        #print(len(self.terr))
        for i in range(num):
            self.terr[random.randint(0,TERRAIN_LENGTH-1)] = self.PIT
    ...
    def set_env_params(self, pit_width=None, stair_width=None, stair_steps=None, stump_height=None):
        ...
        self._update_env_params(**params)
    def _get_terrain_(self, i):
        if(self.terr is not None and i<len(self.terr)):
            return self.terr[i]
        else:
            return self.GRASS
